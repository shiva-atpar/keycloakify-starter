...
const defaultOptions: Country[] = (countriesData as any).all.filter((country: Country) =>
...
const CountryDropdownComponent = (
    ...
    useEffect(() => {
        if (defaultValue) {
            const initial = options.find(c => c.alpha3 === defaultValue);
    ...
    const triggerClasses = cn(
        "flex h-9 w-full items-center justify-between whitespace-nowrap rounded-md border bg-white px-3 py-2 text-sm shadow-sm focus:outline-none",
        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger ref={ref} className={triggerClasses} disabled={disabled} {...props}>
                {selectedCountry ? (
                    ...
                    <span>{slim === false ? (placeholder as string) : (<Globe size={20} />)}</span>
                )}
                <ChevronDown size={16} />
            </PopoverTrigger>
            <PopoverContent collisionPadding={10} side="bottom" className="min-w-[--radix-popper-anchor-width] p-0">
                <Command className="max-h-[270px] w-full">
                    <CommandList>
                        ...
                        <CommandGroup>
                            {options.filter(x => x.name).map((option, key) => (
                                <CommandItem className="flex w-full items-center gap-2" key={key} onSelect={() => handleSelect(option)}>
                                    ...
                                    <CheckIcon
                                        className={cn(
                                            ...
                                            option.alpha2 === selectedCountry?.alpha2 ? "opacity-100" : "opacity-0"
                                        )}
                                    />
                                </CommandItem>
                            ))}
